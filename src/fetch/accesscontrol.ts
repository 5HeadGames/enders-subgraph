...
import {Role, AccessControl, AccessControlRole} from "../../generated/schema";
...
export function fetchAccessControlRole(contract: AccessControl, role: Role): AccessControlRole {
  ...
  if (acr == null) {
    ...
    acr.role = role.id;
    acr.admin =
      role.id == constants.BYTES32_ZERO
        ? acr.id
        : fetchAccessControlRole(
          contract,
          fetchRole(Bytes.fromHexString(constants.BYTES32_ZERO) as Bytes)
        ).id;
    acr.save();
  }
